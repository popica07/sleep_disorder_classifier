...
def save_pooled_confusion_matrix(y_true, y_pred, scenario_name, model_name="RF"):
...
def main():
    ...
    for scenario in SCENARIOS:
        ...
        for seed_key, folds in tqdm(splits_dict.items(), desc=f"RF {sc_name}"):
            ...
            for fold_data in folds:
                ...
                if scenario.get("weights", False) or scenario.get("focal", False):
                    ...
                    class_weight_arg = dict(zip(classes, weights))
        ...
        filename = f"results_rf_{sc_name}_multiseed.pkl"
        out_path = os.path.join(OUT_DIR, filename)
        with open(out_path, 'wb') as f:
            pickle.dump(results_pkg, f)
            
        print(f"Saved {sc_name} metrics to {filename}")

        save_pooled_confusion_matrix(grand_y_true, grand_y_pred, sc_name, model_name="RF")
